...
- 现阶段仅关注桌面端，改动必须在 `1280×720` 复核；页面主体不得横向溢出，所有一级导航（含首页）必须可达。移动端适配与 `390×844` 复核暂缓，除非用户后续明确恢复移动端要求。
...
- 禁止把本地生成的 `.jar`、`.7z`、`.tar.gz`、前端构建包或其他发布包上传到生产服务器。
- 发布只校验本次涉及的服务健康、数据库状态、版本号和必要业务接口；禁止把 Codex 登录状态加入通用发布检查。
