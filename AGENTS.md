...
- 开始发布前检查本机 `ComfyUIAgent` Bridge；若正在运行，必须先停止 Bridge，避免旧进程干扰发布前契约测试和源码边界校验。
- 生产服务器不用于日常开发。发布时由 AWS、腾讯云或其他实际生产服务器从 GitHub 拉取指定的 `master` 提交，并在服务器本机完成测试、前端构建和后端打包。
