...
public class QuantBacktestConfiguration {
  ...
  public ThreadPoolExecutor quantBacktestExecutor(QuantBacktestProperties p) {
    if (p.getExecutorCorePoolSize() < 1
        || p.getExecutorCorePoolSize() > 8
        || p.getExecutorMaxPoolSize() < 1
        || p.getExecutorMaxPoolSize() > 16
        || p.getExecutorQueueCapacity() < 1
        || p.getExecutorQueueCapacity() > 1000
        || p.getExecutorMaxPoolSize() < p.getExecutorCorePoolSize())
      throw new IllegalArgumentException("quant.backtest executor limits invalid");
    return new GracefulBacktestExecutor(
        p.getExecutorCorePoolSize(), p.getExecutorMaxPoolSize(), p.getExecutorQueueCapacity());
  }
  ...
  static final class GracefulBacktestExecutor extends ThreadPoolExecutor {
    ...
    GracefulBacktestExecutor(int core, int max, int capacity) {
      super(
          core,
          max,
          0L,
          ...
          r -> {
            Thread t = new Thread(r, "quant-backtest-" + IDS.incrementAndGet());
            t.setDaemon(false);
            return t;
          },
          new AbortPolicy());
    ...
    public void shutdownGracefully() {
      shutdown();
      try {
        if (!awaitTermination(30, TimeUnit.SECONDS)) shutdownNow();
