...
public class ApiExceptionHandler {
  ...
  public Result<Void> notFound(ResourceNotFoundException exception) {
  ...
  @ResponseStatus(HttpStatus.SERVICE_UNAVAILABLE)
  public Result<Void> remoteCodex(RemoteCodexException exception) {
    return Result.error(503, exception.getMessage());
  }

  @ExceptionHandler(ContentAiException.class)
  @ResponseStatus(HttpStatus.BAD_GATEWAY)
  public Result<Void> contentAi(ContentAiException exception) {
    return Result.error(502, exception.getMessage());
  }

  @ExceptionHandler(ContentSourceException.class)
  @ResponseStatus(HttpStatus.BAD_GATEWAY)
  public Result<Void> contentSource(ContentSourceException exception) {
    return Result.error(502, exception.getMessage());
  }

  @ExceptionHandler(XiaohongshuAutomationException.class)
  @ResponseStatus(HttpStatus.BAD_GATEWAY)
  public Result<Void> xiaohongshuAutomation(XiaohongshuAutomationException exception) {
    return Result.error(502, exception.getMessage());
  }

  @ExceptionHandler(PromptTranslationException.class)
  @ResponseStatus(HttpStatus.BAD_GATEWAY)
  public Result<Void> promptTranslation(PromptTranslationException exception) {
    return Result.error(502, exception.getMessage());
  }
  ...
  public ResponseEntity<Result<Void>> backtest(BacktestTaskException exception) {
    int status;
    switch (exception.getErrorCode()) {
      case "BACKTEST_RUN_NOT_FOUND":
      case "BACKTEST_DATASET_NOT_FOUND":
      case "BACKTEST_EXPERIMENT_NOT_FOUND":
        status = 404;
        break;
      case "BACKTEST_QUEUE_FULL":
        status = 503;
        break;
      case "BACKTEST_STATE_CONFLICT":
      case "BACKTEST_EXPERIMENT_STATE_CONFLICT":
      case "BACKTEST_RUN_ID_CONFLICT":
        status = 409;
        break;
      case "BACKTEST_EXPERIMENT_DISPATCH_FAILED":
        status = 500;
        break;
      case "BACKTEST_RESULT_INVALID":
      case "BACKTEST_EXECUTION_FAILED":
      case "BACKTEST_PERSISTENCE_FAILED":
        status = 500;
        break;
      default:
        status = 400;
  ...
  @ExceptionHandler(WalkForwardTaskException.class)
  public ResponseEntity<Result<Void>> walkForward(WalkForwardTaskException exception) {
    int status;
    switch (exception.getErrorCode()) {
      ...
      case "WALK_FORWARD_NOT_TERMINAL":
        status = 409;
        break;
      case "WALK_FORWARD_DISPATCH_FAILED":
      case "WALK_FORWARD_OOS_INVALID":
        status = 500;
        break;
      default:
        status = 400;
  ...
  private static final class StateError {
    ...
    private StateError(int status, String message) {
      this.status = status;
