...
public interface AsrRecordMapper {
    @Insert("INSERT INTO c_AsrTranscriptionRecords(RequestId,SessionId,AudioPath,AudioFormat,AudioSize,Provider,Model,Language,Status) VALUES(#{requestId},#{sessionId},#{audioPath},#{audioFormat},#{audioSize},#{provider},#{model},#{language},'PROCESSING')")
    ...
    @Select("<script>SELECT Id id,RecordId recordId,RequestId requestId,SessionId sessionId,AudioFormat audioFormat,AudioSize audioSize,AudioDurationMs audioDurationMs,RecognizedText recognizedText,CorrectedText correctedText,Provider provider,Model model,Language language,ProcessingTimeMs processingTimeMs,Status status,ErrorCode errorCode,ErrorMessage errorMessage,CreatedAt createdAt FROM c_AsrTranscriptionRecords <where><if test='status != null'>AND Status=#{status}</if><if test='provider != null'>AND Provider=#{provider}</if><if test='model != null'>AND Model=#{model}</if><if test='keyword != null'>AND (RecognizedText LIKE CONCAT('%',#{keyword},'%') OR CorrectedText LIKE CONCAT('%',#{keyword},'%'))</if><if test='startTime != null'>AND CreatedAt &gt;= #{startTime}</if><if test='endTime != null'>AND CreatedAt &lt;= #{endTime}</if></where> ORDER BY CreatedAt DESC,Id DESC LIMIT #{offset},#{limit}</script>")
    List<Map<String,Object>> findPage(@Param("status") String status,@Param("provider") String provider,@Param("model") String model,@Param("keyword") String keyword,@Param("startTime") LocalDateTime startTime,@Param("endTime") LocalDateTime endTime,@Param("offset") int offset,@Param("limit") int limit);

    @Select("<script>SELECT COUNT(*) FROM c_AsrTranscriptionRecords <where><if test='status != null'>AND Status=#{status}</if><if test='provider != null'>AND Provider=#{provider}</if><if test='model != null'>AND Model=#{model}</if><if test='keyword != null'>AND (RecognizedText LIKE CONCAT('%',#{keyword},'%') OR CorrectedText LIKE CONCAT('%',#{keyword},'%'))</if><if test='startTime != null'>AND CreatedAt &gt;= #{startTime}</if><if test='endTime != null'>AND CreatedAt &lt;= #{endTime}</if></where></script>")
    long count(@Param("status") String status,@Param("provider") String provider,@Param("model") String model,@Param("keyword") String keyword,@Param("startTime") LocalDateTime startTime,@Param("endTime") LocalDateTime endTime);
