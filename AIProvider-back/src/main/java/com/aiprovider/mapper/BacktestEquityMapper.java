...
import com.aiprovider.mapper.row.BacktestEquityRow; import org.apache.ibatis.annotations.*; import java.util.List;
@Mapper public interface BacktestEquityMapper {
 ...
 @ResultMap("backtestEquityRow") @Select("SELECT " + COLUMNS + " FROM q_backtest_equity WHERE RunId=#{runId} ORDER BY PointIndex ASC") List<BacktestEquityRow> findAll(@Param("runId") String runId);
