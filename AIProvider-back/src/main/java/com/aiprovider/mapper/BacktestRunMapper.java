...
public interface BacktestRunMapper {
  ...
  @Results(
      ...
      value = {
        ...
        @Result(column = "DatasetLastValidatedAt", property = "datasetLastValidatedAt"),
        @Result(column = "DatasetLastSyncTaskId", property = "datasetLastSyncTaskId"),
        @Result(column = "Provider", property = "provider"),
        @Result(column = "MarketType", property = "marketType"),
        @Result(column = "DataType", property = "dataType"),
        @Result(column = "Symbol", property = "symbol"),
        @Result(column = "IntervalCode", property = "intervalCode"),
        @Result(column = "StartOpenTimeMs", property = "startOpenTimeMs"),
        ...
        @Result(column = "BarCount", property = "barCount"),
        @Result(column = "TradeCount", property = "tradeCount"),
        @Result(column = "WinningTradeCount", property = "winningTradeCount"),
        @Result(column = "LosingTradeCount", property = "losingTradeCount"),
        @Result(column = "BreakEvenTradeCount", property = "breakEvenTradeCount"),
        @Result(column = "WinRate", property = "winRate"),
        @Result(column = "GrossProfit", property = "grossProfit"),
        @Result(column = "GrossLoss", property = "grossLoss"),
        @Result(column = "NetProfit", property = "netProfit"),
        @Result(column = "TotalReturnRatio", property = "totalReturnRatio"),
        @Result(column = "MaximumDrawdownRatio", property = "maximumDrawdownRatio"),
        @Result(column = "ProfitFactor", property = "profitFactor"),
  ...
  @Select(
      "<script>SELECT "
          + COLUMNS
          + " FROM q_backtest_run WHERE RunId IN <foreach collection='runIds' item='runId' open='('"
          + " separator=',' close=')'>#{runId}</foreach></script>")
  List<BacktestRunRow> findByRunIdsSql(@Param("runIds") List<String> runIds);
  ...
  @Select(
      "<script>SELECT "
          + COLUMNS
          + " FROM q_backtest_run WHERE 1=1 <if test='status != null'>AND Status=#{status}</if><if"
          + " test='symbol != null'>AND Symbol=#{symbol}</if><if test='strategyCode != null'>AND"
          + " StrategyCode=#{strategyCode}</if> ORDER BY QueuedAt DESC, Id DESC LIMIT #{limit}"
          + " OFFSET #{offset}</script>")
  List<BacktestRunRow> findPage(
  ...
  @Select(
      "<script>SELECT COUNT(*) FROM q_backtest_run WHERE 1=1 <if test='status != null'>AND"
  ...
  @Select(
      "SELECT "
          + COLUMNS
          + " FROM q_backtest_run WHERE Status IN"
          + " ('QUEUED','LOADING_SNAPSHOT','RUNNING_ENGINE','PERSISTING') ORDER BY QueuedAt ASC, Id"
          + " ASC")
  List<BacktestRunRow> findNonTerminal();

  @Update(
      "UPDATE q_backtest_run SET"
          + " Status=#{toStatus},ProgressPercent=#{progress},UpdatedAt=#{now},StartedAt=IF(#{toStatus}='LOADING_SNAPSHOT',#{now},StartedAt)"
          + " WHERE RunId=#{runId} AND Status=#{fromStatus}")
  int transition(
      @Param("runId") String runId,
  ...
  @Update(
      "UPDATE q_backtest_run SET"
          + " DatasetLastValidatedAt=#{datasetLastValidatedAt},DatasetLastSyncTaskId=#{datasetLastSyncTaskId},Provider=#{provider},MarketType=#{marketType},DataType=#{dataType},Symbol=#{symbol},IntervalCode=#{intervalCode},BarCount=#{barCount},UpdatedAt=#{updatedAt}"
          + " WHERE RunId=#{runId} AND Status='LOADING_SNAPSHOT'")
  int updateSnapshot(BacktestRunRow row);
  ...
  @Update(
      "UPDATE q_backtest_run SET"
          ...
          + " WHERE RunId=#{runId} AND Status NOT IN ('COMPLETED','FAILED')")
  int fail(
      @Param("runId") String runId,
