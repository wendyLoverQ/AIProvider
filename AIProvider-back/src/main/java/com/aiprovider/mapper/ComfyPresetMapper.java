...
public interface ComfyPresetMapper {
    @Select("SELECT Id id, Title title, ParametersJson parametersJson, OutputFolder outputFolder, Notes notes, IsDefault isDefault FROM c_ComfyParameterSchemes ORDER BY IsDefault DESC, UpdatedAt DESC")
    List<Map<String, Object>> findAll();

    @Insert("INSERT INTO c_ComfyParameterSchemes(Title, ParametersJson, OutputFolder, Notes) VALUES(#{title}, CAST(#{parametersJson} AS JSON), #{outputFolder}, #{notes})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    void insert(PresetInsert preset);

    @Update("UPDATE c_ComfyParameterSchemes SET Title=#{title}, ParametersJson=CAST(#{parametersJson} AS JSON), OutputFolder=#{outputFolder}, Notes=#{notes} WHERE Id=#{id}")
