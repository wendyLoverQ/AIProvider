...
public interface ContentOperationsMapper {
    ...
    @Select("SELECT Id id,PlatformAccountId platformAccountId,Platform platform,DisplayName displayName,AccountHandle accountHandle,PublishMode publishMode,AdapterType adapterType,Enabled enabled,ConnectionStatus connectionStatus,AdapterStatus adapterStatus,LastError lastError,LastConnectedAt lastConnectedAt,LastPublishedAt lastPublishedAt FROM c_ContentAccounts WHERE Platform='XIAOHONGSHU' AND ArchivedAt IS NULL ORDER BY Enabled DESC,UpdatedAt DESC")
    List<Map<String,Object>> findAccounts();

    @Select("SELECT Id id,PlatformAccountId platformAccountId,Platform platform,DisplayName displayName,AccountHandle accountHandle,PublishMode publishMode,AdapterType adapterType,Enabled enabled,ConnectionStatus connectionStatus,AdapterStatus adapterStatus,LastError lastError,LastConnectedAt lastConnectedAt,LastPublishedAt lastPublishedAt FROM c_ContentAccounts WHERE Id=#{id} AND Platform='XIAOHONGSHU' AND ArchivedAt IS NULL")
    ...
    @Select("SELECT Id id,PlatformAccountId platformAccountId,Platform platform,DisplayName displayName,AdapterType adapterType,Enabled enabled FROM c_ContentCollectionAccounts WHERE ArchivedAt IS NULL ORDER BY Enabled DESC,UpdatedAt DESC")
    List<Map<String,Object>> findCollectionAccounts();

    @Select("SELECT Id id,PlatformAccountId platformAccountId,Platform platform,DisplayName displayName,AdapterType adapterType,Enabled enabled FROM c_ContentCollectionAccounts WHERE Id=#{id} AND ArchivedAt IS NULL")
    ...
    @Select("SELECT p.Id id,p.Status status,p.AccountId accountId,p.DraftId draftId,a.PlatformAccountId platformAccountId,a.DisplayName displayName,a.AccountHandle accountHandle,a.Enabled accountEnabled,d.ContentItemId contentItemId,d.Title title,d.Body body,d.TagsJson tagsJson FROM c_ContentPublications p JOIN c_ContentAccounts a ON a.Id=p.AccountId JOIN c_ContentDrafts d ON d.Id=p.DraftId WHERE p.Id=#{id} AND d.Platform='XIAOHONGSHU'")
