...
public interface ResearchStudyMapper {
  String COLUMNS = "r.Id,r.ResearchStudyId,r.Name,r.Description,r.DatasetId,r.Provider,r.MarketType,r.DataType,r.Symbol,r.IntervalCode,r.StrategyCode,r.StrategyVersion,r.ExecutionProfileCode,r.DirectionMode,r.OrderSizingMode,r.EvaluationMode,r.ParameterSpaceMode,r.ParameterSpaceJson,r.ExpandedParameterGridJson,r.CandidateCount,r.StudyStartOpenTimeMs,r.StudyEndOpenTimeMs,r.TrainingBars,r.ValidationBars,r.SelectionMetric,r.MinimumTrainTrades,r.InitialCapital,r.OrderAmount,r.FeeRate,r.ForceCloseAtEnd,r.ComparisonGroupKey,r.WalkForwardStudyId,r.Status,r.ProgressPercent,r.ErrorCode,r.ErrorMessage,r.CreatedAt,r.StartedAt,r.FinishedAt,r.UpdatedAt,w.SuccessfulOosFolds AS OosSuccessfulOosFolds,w.FailedFolds AS OosFailedFolds,w.HasOosGaps AS OosHasOosGaps,w.OosTotalReturnRatio AS OosTotalReturnRatio,w.OosMaximumDrawdownRatio AS OosMaximumDrawdownRatio,w.OosTradeCount AS OosTradeCount,w.OosTotalFees AS OosTotalFees,w.ParameterChanges AS OosParameterChanges,w.OosAggregateVersion AS OosAggregateVersion";
  ...
  @Insert("INSERT INTO q_research_study(ResearchStudyId,Name,Description,DatasetId,Provider,MarketType,DataType,Symbol,IntervalCode,StrategyCode,StrategyVersion,ExecutionProfileCode,DirectionMode,OrderSizingMode,EvaluationMode,ParameterSpaceMode,ParameterSpaceJson,ExpandedParameterGridJson,CandidateCount,StudyStartOpenTimeMs,StudyEndOpenTimeMs,TrainingBars,ValidationBars,SelectionMetric,MinimumTrainTrades,InitialCapital,OrderAmount,FeeRate,ForceCloseAtEnd,ComparisonGroupKey,WalkForwardStudyId,Status,ProgressPercent,CreatedAt,UpdatedAt) VALUES(#{researchStudyId},#{name},#{description},#{datasetId},#{provider},#{marketType},#{dataType},#{symbol},#{intervalCode},#{strategyCode},#{strategyVersion},#{executionProfileCode},#{directionMode},#{orderSizingMode},#{evaluationMode},#{parameterSpaceMode},#{parameterSpaceJson},#{expandedParameterGridJson},#{candidateCount},#{studyStartOpenTimeMs},#{studyEndOpenTimeMs},#{trainingBars},#{validationBars},#{selectionMetric},#{minimumTrainTrades},#{initialCapital},#{orderAmount},#{feeRate},#{forceCloseAtEnd},#{comparisonGroupKey},#{walkForwardStudyId},'QUEUED',0,#{createdAt},#{updatedAt})")
