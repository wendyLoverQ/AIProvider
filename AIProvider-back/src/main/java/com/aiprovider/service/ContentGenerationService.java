...
public class ContentGenerationService {
    ...
    com.aiprovider.logging.BusinessOperationLogger.start("service.ContentGenerationService.rewrite", new String[] { "dto" }, new Object[] { dto });
    if(dto==null)throw new IllegalArgumentException("内容改写请求不能为空");String source=required(dto.getSourceText(),"来源内容",30000);
        ...
        GeminiRuntimeConfig config=configService.runtime();return com.aiprovider.logging.BusinessOperationLogger.success("service.ContentGenerationService.rewrite", execute("CONTENT_REWRITE",config,config.contentRewritePrompt,user,input));
    }

    public ContentGenerationVO reply(CommentReplyRequestDTO dto){
    com.aiprovider.logging.BusinessOperationLogger.start("service.ContentGenerationService.reply", new String[] { "dto" }, new Object[] { dto });
    if(dto==null)throw new IllegalArgumentException("评论回复请求不能为空");String comment=required(dto.getCommentText(),"用户评论",10000);
        ...
        GeminiRuntimeConfig config=configService.runtime();return com.aiprovider.logging.BusinessOperationLogger.success("service.ContentGenerationService.reply", execute("COMMENT_REPLY",config,config.commentReplyPrompt,user,input));
    }

    public ContentGenerationVO testConnection(){
        com.aiprovider.logging.BusinessOperationLogger.start("service.ContentGenerationService.testConnection", new String[] {}, new Object[] {});
        GeminiRuntimeConfig config=configService.runtime();Map<String,Object> input=new LinkedHashMap<>();input.put("purpose","connection_test");return com.aiprovider.logging.BusinessOperationLogger.success("service.ContentGenerationService.testConnection", execute("CONNECTION_TEST",config,"你是连接测试助手。严格按用户要求返回。","只回复：连接成功",input));}
