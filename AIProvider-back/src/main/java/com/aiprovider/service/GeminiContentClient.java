...
public class GeminiContentClient {
    ...
    public boolean validateApiKey(String apiBaseUrl,String apiKey){
        com.aiprovider.logging.BusinessOperationLogger.start("service.GeminiContentClient.validateApiKey", new String[] { "apiBaseUrl", "apiKey" }, new Object[] { apiBaseUrl, apiKey });
        HttpHeaders headers=new HttpHeaders();headers.set("x-goog-api-key",apiKey);try{ResponseEntity<JsonNode> response=http.exchange(URI.create(apiBaseUrl.replaceAll("/+$","")+"/v1beta/models?pageSize=1"),HttpMethod.GET,new HttpEntity<>(headers),JsonNode.class);return com.aiprovider.logging.BusinessOperationLogger.success("service.GeminiContentClient.validateApiKey", response.getStatusCode().is2xxSuccessful()&&response.getBody()!=null&&response.getBody().path("models").isArray());}catch(HttpStatusCodeException e){throw new ContentAiException("GEMINI_HTTP_"+e.getRawStatusCode(),"Gemini 凭据验证失败（HTTP "+e.getRawStatusCode()+"）");}catch(RestClientException e){throw new ContentAiException("GEMINI_UNAVAILABLE","Gemini 服务不可用或请求超时",e);}}
    public String generate(GeminiRuntimeConfig config,String systemPrompt,String userPrompt){
    com.aiprovider.logging.BusinessOperationLogger.start("service.GeminiContentClient.generate", new String[] { "config", "systemPrompt", "userPrompt" }, new Object[] { config, systemPrompt, userPrompt });
    return com.aiprovider.logging.BusinessOperationLogger.success("service.GeminiContentClient.generate", generate(config,systemPrompt,userPrompt,false));
    }
    public String generateJson(GeminiRuntimeConfig config,String systemPrompt,String userPrompt){
    com.aiprovider.logging.BusinessOperationLogger.start("service.GeminiContentClient.generateJson", new String[] { "config", "systemPrompt", "userPrompt" }, new Object[] { config, systemPrompt, userPrompt });
    return com.aiprovider.logging.BusinessOperationLogger.success("service.GeminiContentClient.generateJson", generate(config,systemPrompt,userPrompt,true,null));
    }
    public String generateDraftJson(GeminiRuntimeConfig config,String systemPrompt,String userPrompt){
    com.aiprovider.logging.BusinessOperationLogger.start("service.GeminiContentClient.generateDraftJson", new String[] { "config", "systemPrompt", "userPrompt" }, new Object[] { config, systemPrompt, userPrompt });
    ObjectNode schema=json.createObjectNode();schema.put("type","object");ObjectNode properties=schema.putObject("properties");properties.putObject("title").put("type","string").put("maxLength",20);properties.putObject("body").put("type","string").put("maxLength",1000);properties.putObject("tags").put("type","array").put("maxItems",10).putObject("items").put("type","string").put("maxLength",30);schema.putArray("required").add("title").add("body").add("tags");schema.put("additionalProperties",false);return com.aiprovider.logging.BusinessOperationLogger.success("service.GeminiContentClient.generateDraftJson", generate(config,systemPrompt,userPrompt,true,schema));
    }
