...
public class MonitorService {
    ...
    com.aiprovider.logging.BusinessOperationLogger.start("service.MonitorService.summary", new String[] {}, new Object[] {});
    ResourceCache resources=resources(); HealthService.Snapshot state=health.check();
        ...
        return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.summary", new MonitorSummaryVO(new MonitorSummaryVO.Health(state.getStatus(),state.getCheckedAt()),OffsetDateTime.now(zone),resources.memory,resources.disk,localTraffic));
    }
    ...
    com.aiprovider.logging.BusinessOperationLogger.start("service.MonitorService.overview", new String[] {}, new Object[] {});
    Instant now=Instant.now();OverviewCache value=overviewCache;
        if(value!=null&&Duration.between(value.at,now).compareTo(OVERVIEW_TTL)<0)return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.overview", value.data);
        synchronized(overviewLock){value=overviewCache;now=Instant.now();if(value!=null&&Duration.between(value.at,now).compareTo(OVERVIEW_TTL)<0)return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.overview", value.data);
            ...
            overviewCache=new OverviewCache(now,Collections.unmodifiableMap(data));return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.overview", overviewCache.data);}
    }
    public List<Map<String,Object>> timeseries(String range){
    com.aiprovider.logging.BusinessOperationLogger.start("service.MonitorService.timeseries", new String[] { "range" }, new Object[] { range });
    int hours=hours(range);List<Map<String,Object>> rows=repository.timeseries(hours);Map<String,Object> p95=new HashMap<>();
        ...
        return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.timeseries", rows);
    }
    public Map<String,Object> providers(){
        com.aiprovider.logging.BusinessOperationLogger.start("service.MonitorService.providers", new String[] {}, new Object[] {});
        Map<String,Object> result=new LinkedHashMap<>();result.put("selection",repository.selection());result.put("activity",repository.providerActivity());
        HealthService.Snapshot snapshot=health.check();result.put("dependencies",Collections.singletonList(dependency("database",snapshot.getStatus(),snapshot.getCheckedAt())));return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.providers", result);}
    ...
    com.aiprovider.logging.BusinessOperationLogger.start("service.MonitorService.failures", new String[] { "range", "provider", "model", "page", "pageSize" }, new Object[] { range, provider, model, page, pageSize });
    if(page<1)throw new IllegalArgumentException("page 必须大于等于 1");if(pageSize<1||pageSize>100)throw new IllegalArgumentException("pageSize 必须在 1 到 100 之间");int hours=hours(range);
        ...
        return com.aiprovider.logging.BusinessOperationLogger.success("service.MonitorService.failures", new MonitorPageVO<>(items,repository.failureCount(hours,cleanFilter(provider),cleanFilter(model)),page,pageSize));
    }
