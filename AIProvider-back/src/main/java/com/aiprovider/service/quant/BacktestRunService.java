...
public class BacktestRunService {
  ...
  private final MarketDatasetRepository datasets;
  ...
  public BacktestRunService(
      BacktestRunMapper r,
      BacktestTradeMapper t,
      BacktestEquityMapper e,
      MarketDatasetRepository d,
  ...
  String createWithRunId(String id, BacktestCreateRequest q) {
    try {
      ...
      throw error("BACKTEST_RUN_ID_CONFLICT", "runId must be a UUID");
    }
    validate(q);
    ...
    QuantStrategyDefinition def = definition(code);
    ...
    row.startOpenTimeMs = q.getStartOpenTimeInclusive().toEpochMilli();
    ...
    try {
      if (runs.insert(row) != 1) {
        throw error(
            "BACKTEST_RUN_PERSISTENCE_FAILED", "run insert affected an unexpected number of rows");
      }
  ...
  private void run(BacktestRunCommand c) {
    try {
      ...
      MarketDataSnapshot s =
          snapshots.load(c.datasetId(), c.startOpenTimeInclusive(), c.endOpenTimeExclusive());
      BacktestRunRow meta = new BacktestRunRow();
      meta.runId = c.runId();
      meta.datasetLastValidatedAt = s.getDatasetLastValidatedAt();
      meta.datasetLastSyncTaskId = s.getDatasetLastSyncTaskId();
      meta.provider = s.getProvider().name();
      meta.marketType = s.getMarketType().name();
      meta.dataType = s.getDataType().name();
      meta.symbol = s.getSymbol();
      meta.intervalCode = s.getInterval().code();
      meta.barCount = (int) s.getActualCandleCount();
      meta.updatedAt = Instant.now();
      if (runs.updateSnapshot(meta) != 1)
        throw error("BACKTEST_STATE_CONFLICT", "runId=" + c.runId() + " snapshot update failed");
      cas(
          c.runId(),
          BacktestRunStatus.LOADING_SNAPSHOT,
      ...
      cas(
          c.runId(),
          BacktestRunStatus.RUNNING_ENGINE,
          BacktestRunStatus.PERSISTING,
          new BigDecimal("80"));
      persistence.persistCompleted(c.runId(), result);
    } catch (MarketDataSnapshotException e) {
      failSafely(
          c.runId(),
          "BACKTEST_SNAPSHOT_INVALID",
          "runId=" + c.runId() + " snapshotCode=" + e.getErrorCode() + " " + e.getMessage());
    } catch (BacktestException | StrategyException | BacktestTaskException e) {
      failSafely(
          c.runId(),
          e instanceof BacktestException
              ? ((BacktestException) e).getErrorCode()
              : e instanceof StrategyException
                  ? ((StrategyException) e).getErrorCode()
                  : ((BacktestTaskException) e).getErrorCode(),
          descriptor(e.getMessage()));
    } catch (RuntimeException e) {
      failSafely(c.runId(), "BACKTEST_EXECUTION_FAILED", descriptor(e.getMessage()));
    }
  }

  private void failSafely(String id, String code, String message) {
    try {
      failures.markFailed(id, code, message);
    } catch (RuntimeException ignored) {
    }
  }

  private String descriptor(String message) {
    String s = message == null ? "backtest failed" : message.replaceAll("[\\r\\n]", " ");
    return s.substring(0, Math.min(1000, s.length()));
  }

  private void cas(String id, BacktestRunStatus from, BacktestRunStatus to, BigDecimal progress) {
    if (runs.transition(id, from.name(), to.name(), progress, Instant.now()) != 1)
  ...
  private static long calculateOffset(int page, int size) {
    try {
      long offset = Math.multiplyExact((long) page - 1L, (long) size);
      if (offset > MAX_QUERY_OFFSET)
        throw new BacktestTaskException("BACKTEST_REQUEST_INVALID", "page offset exceeds limit");
      return offset;
    } catch (ArithmeticException e) {
      throw new BacktestTaskException("BACKTEST_REQUEST_INVALID", "page offset overflow", e);
    }
  }

  private void validate(BacktestCreateRequest q) {
    if (q == null
        || q.getDatasetId() <= 0
        || q.getStartOpenTimeInclusive() == null
        || q.getEndOpenTimeExclusive() == null
        || !q.getStartOpenTimeInclusive().isBefore(q.getEndOpenTimeExclusive())
        || q.getStrategyCode() == null
        ...
        || q.getFeeRate() == null
        || !fitsDecimal38_18(q.getFeeRate())
        || q.getFeeRate().signum() < 0
        || q.getFeeRate().compareTo(new BigDecimal("0.01")) > 0
        || !q.isForceCloseAtEnd()
        || q.getStrategyParameters() != null
            && q.getStrategyParameters().entrySet().stream()
                .anyMatch(e -> e.getKey() == null || e.getValue() == null))
      throw error("BACKTEST_REQUEST_INVALID", "invalid backtest request");
  }

  private static boolean fitsDecimal38_18(BigDecimal v) {
    if (v == null || v.scale() > 18 || v.precision() > 38) return false;
    BigDecimal n = v.stripTrailingZeros();
    return Math.max(0, n.precision() - n.scale()) <= 20;
  }

  private BacktestRunRow require(String id) {
    BacktestRunRow r = runs.findByRunId(id);
    if (r == null) throw error("BACKTEST_RUN_NOT_FOUND", "runId=" + id + " not found");
    return r;
  }

  private QuantStrategyDefinition definition(String code) {
    try {
      return strategies.get(code == null ? null : code.trim());
  ...
  private BacktestTaskException error(String c, String m) {
    return new BacktestTaskException(c, m);
  }
  ...
  private String write(Object v) {
    try {
      ...
      throw error("BACKTEST_REQUEST_INVALID", "parameters cannot be serialized");
    }
  }

  private BacktestDtos.RunDetail detail(BacktestRunRow r) {
    Map<String, Integer> req = read(r.requestedParametersJson),
        res = r.resolvedParametersJson == null ? Map.of() : read(r.resolvedParametersJson);
    Map<String, Object> m = new LinkedHashMap<>();
    m.put("winRate", r.winRate);
    m.put("grossProfit", r.grossProfit);
    m.put("grossLoss", r.grossLoss);
    m.put("netProfit", r.netProfit);
    m.put("totalReturnRatio", r.totalReturnRatio);
    m.put("maximumDrawdownRatio", r.maximumDrawdownRatio);
    m.put("profitFactor", r.profitFactor);
    ...
    return new BacktestDtos.RunDetail(
        r.runId,
        r.datasetId,
        r.datasetLastValidatedAt,
        r.datasetLastSyncTaskId,
        r.provider,
        r.marketType,
        r.dataType,
        r.symbol,
        r.intervalCode,
        Instant.ofEpochMilli(r.startOpenTimeMs),
        ...
        m,
        r.equityPointCount == null ? 0 : r.equityPointCount);
  }

  private BacktestDtos.Trade trade(BacktestTradeRow r) {
    return new BacktestDtos.Trade(
        r.tradeNo,
        r.entrySignalIndex,
        r.entryIndex,
        Instant.ofEpochMilli(r.entryTimeMs),
        r.entryPrice,
        r.exitSignalIndex,
        r.exitIndex,
        Instant.ofEpochMilli(r.exitTimeMs),
        r.exitPrice,
        r.amount,
        r.grossProfit,
        r.fee,
  ...
  private BacktestDtos.EquityPoint point(BacktestEquityRow r) {
    return new BacktestDtos.EquityPoint(
        r.pointIndex,
  ...
  private Map<String, Integer> read(String s) {
    try {
      return json.readValue(s, new TypeReference<Map<String, Integer>>() {});
    } catch (Exception e) {
      throw error("BACKTEST_PERSISTENCE_FAILED", "stored JSON is invalid");
    }
  }

  private List<String> readWarnings(String s) {
    if (s == null || s.isBlank()) return List.of();
    try {
      return List.copyOf(json.readValue(s, new TypeReference<List<String>>() {}));
    } catch (Exception e) {
      throw error("BACKTEST_PERSISTENCE_FAILED", "stored warnings JSON is invalid");
    }
  }
