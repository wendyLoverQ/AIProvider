...
class QuantBacktestExperimentControllerContractTest {
    @Test void exposesTheFourFrozenExperimentEndpoints() throws Exception {
        BacktestExperimentService service=mock(BacktestExperimentService.class);
        when(service.create(any())).thenReturn(new BacktestExperimentDtos.CreateResponse("e",2,4));
        MockMvc mvc=standaloneSetup(new QuantBacktestExperimentController(service)).build();
