...
class AsrTranscriptionServiceTest {
    ...
    @Test void persistsAndReturnsSuccessfulTranscriptionWithoutLoggingContent() throws Exception {
        ...
        doAnswer(invocation->{AsrRecordMapper.Record row=invocation.getArgument(0);row.setId(7L);return 1;}).when(repository).insert(any());when(repository.assignRecordId(eq(7L),anyString())).thenReturn(1);when(client.transcribe(any(),eq("whisper-large-v3"),eq("zh"))).thenReturn(new AsrProviderClient.Result("今天天气很好",8420L,2000L,1997L,"3h"));when(repository.markSuccess(eq(7L),eq("今天天气很好"),eq(8420L),anyLong(),eq(2000L),eq(1997L),eq("3h"))).thenReturn(1);when(repository.findByRecordId(anyString())).thenReturn(successRow());
        AsrRecordVO result=service.transcribe(audio(),"session_001","zh","request_001");
        assertEquals("今天天气很好",result.getRecognizedText());verify(client).transcribe(any(),eq("whisper-large-v3"),eq("zh"));verify(repository).markSuccess(eq(7L),eq("今天天气很好"),eq(8420L),anyLong(),eq(2000L),eq(1997L),eq("3h"));
    }
    ...
    @Test void replaysCompletedRequestWithoutCallingProviderAgain() throws Exception {
        ...
        AsrRecordVO result=service.transcribe(audio(),null,"zh","request_001");
    ...
    @Test void transcribesAudioWithoutCharacterMetadata() throws Exception {
        AsrRecordRepository repository=mock(AsrRecordRepository.class);AsrProviderClient client=mock(AsrProviderClient.class);AsrTranscriptionService service=service(repository,client);when(repository.findByRequestId(anyString())).thenReturn(null);
        doAnswer(invocation->{AsrRecordMapper.Record row=invocation.getArgument(0);row.setId(8L);return 1;}).when(repository).insert(any());when(repository.assignRecordId(eq(8L),anyString())).thenReturn(1);when(client.transcribe(any(),eq("whisper-large-v3"),eq("zh"))).thenReturn(new AsrProviderClient.Result("纯音频转写",1000L,null,null,null));when(repository.markSuccess(eq(8L),eq("纯音频转写"),eq(1000L),anyLong(),isNull(),isNull(),isNull())).thenReturn(1);when(repository.findByRecordId(anyString())).thenReturn(successRow());
        service.transcribe(audio(),null,"zh","request_audio_only");verify(client).transcribe(any(),eq("whisper-large-v3"),eq("zh"));
    }
    ...
    @Test void recordsProviderFailureAndReturnsStablePublicError() throws Exception {
        ...
        AsrTranscriptionException error=assertThrows(AsrTranscriptionException.class,()->service.transcribe(audio(),null,"zh","request_001"));
    ...
    @Test void reportsRecordedDailyRequestCountProviderSnapshotAndAudioUsage(){AsrRecordRepository repository=mock(AsrRecordRepository.class);AsrTranscriptionService service=service(repository,mock(AsrProviderClient.class));Map<String,Object> snapshot=new HashMap<>();snapshot.put("requestLimit",2000L);snapshot.put("requestsRemaining",1997L);snapshot.put("requestsResetAfter","3h");snapshot.put("capturedAt",LocalDateTime.of(2026,7,22,12,0));when(repository.findLatestQuotaSnapshot("groq","whisper-large-v3")).thenReturn(snapshot);when(repository.sumAudioDurationMs(eq("groq"),eq("whisper-large-v3"),any(),any())).thenReturn(4500L,12500L);when(repository.countRequests(eq("groq"),eq("whisper-large-v3"),any(),any())).thenReturn(8L);AsrQuotaVO quota=service.quota();assertEquals(8L,quota.getDailyRequestCount());assertEquals(1997L,quota.getDailyRequestsRemaining());assertEquals(5L,quota.getHourlyAudioUsedSeconds());assertEquals(13L,quota.getDailyAudioUsedSeconds());assertEquals(7195L,quota.getHourlyAudioRemainingSeconds());assertEquals(28787L,quota.getDailyAudioRemainingSeconds());assertEquals("AIPROVIDER_RECORDED",quota.getAudioUsageScope());}
