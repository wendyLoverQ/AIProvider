...
class BacktestExperimentDispatcherTest {
    @Test void doesNotClaimWhenExperimentCapacityIsFull(){
        ...
        when(runs.findByRunIds(anyList())).thenReturn(List.of(run("t1"),run("v1"),run("t2"),run("v2")));
        new BacktestExperimentDispatcher(experiments,candidates,runs,mock(BacktestRunService.class),aggregate,properties(2),new ObjectMapper()).tick();
    ...
    @Test void usesFixedIdsAndMarksCandidateOnlyAfterBothRunsAreCreated(){
        ...
        when(experiments.findNonTerminal()).thenReturn(List.of(experiment));when(candidates.findAll(anyString())).thenReturn(List.of());when(candidates.claimNextPending(anyString(),anyString(),any())).thenReturn(1);when(candidates.findClaimed(anyString(),anyString())).thenReturn(candidate);when(candidates.markDispatched(anyString(),anyString(),any())).thenReturn(1);
    ...
    @Test void aConcurrentCasMissStopsTheTick(){
        BacktestExperimentMapper experiments=mock(BacktestExperimentMapper.class);BacktestExperimentCandidateMapper candidates=mock(BacktestExperimentCandidateMapper.class);when(experiments.findNonTerminal()).thenReturn(List.of(row()));when(candidates.findAll(anyString())).thenReturn(List.of());when(candidates.claimNextPending(anyString(),anyString(),any())).thenReturn(0);
    ...
    private BacktestExperimentCandidateRow candidate(BacktestExperimentRow experiment,String id,String train,String validation){BacktestExperimentCandidateRow c=new BacktestExperimentCandidateRow();c.candidateId=id;c.experimentId=experiment.experimentId;c.trainingRunId=train;c.validationRunId=validation;c.dispatchStatus="DISPATCHED";return c;}
    private BacktestRunRow run(String id){BacktestRunRow run=new BacktestRunRow();run.runId=id;run.status="RUNNING";run.progressPercent=java.math.BigDecimal.TEN;return run;}
