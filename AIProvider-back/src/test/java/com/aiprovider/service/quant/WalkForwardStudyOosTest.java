...
class WalkForwardStudyOosTest {
  ...
  private BacktestRunRow run(String id, String returnRatio, String fees, int trades) { BacktestRunRow row = new BacktestRunRow(); row.runId = id; row.status = "COMPLETED"; row.totalReturnRatio = new BigDecimal(returnRatio); row.totalFees = new BigDecimal(fees); row.tradeCount = trades; row.maximumDrawdownRatio = new BigDecimal("0.05"); return row; }
