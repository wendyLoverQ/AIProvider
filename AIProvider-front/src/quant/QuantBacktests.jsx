...
import {
  calculateExpectedBars, formatDecimal, formatInstant, formatRatio, formatRunStatus, intervalDurationMs, toUtcIso,
...
const pageTotal = (data, items) => (Array.isArray(data) ? items.length : data?.total ?? items.length);
const localInput = (iso) => iso ? new Date(iso).toISOString().slice(0, 16) : "";
...
function CreatePanel({ strategies, datasets, onClose, onCreated }) {
  ...
  const dataset = datasets.find((item) => String(item.datasetId) === String(datasetId));
  ...
  const selectDataset = (value) => {
    const selected = datasets.find((item) => String(item.datasetId) === String(value));
    setDatasetId(value);
    if (!selected) return;
    const duration = intervalDurationMs(selected.interval);
    setStart(localInput(selected.earliestOpenTime));
    setEnd(duration ? localInput(new Date(new Date(selected.latestOpenTime).getTime() + duration).toISOString()) : "");
  };
  ...
  const submit = async (event) => {
    ...
    if (!/^(?:0|[1-9]\d*)(?:\.\d+)?$/.test(orderAmount) || Number(orderAmount) <= 0) return setError("数量必须大于 0");
    if (!/^(?:0|0\.\d+|1(?:\.0+)?)$/.test(feeRate) || Number(feeRate) > 0.01) return setError("手续费必须在 0～0.01 之间");
    setSaving(true);
    try {
      const run = await createBacktestRun({ datasetId: Number(datasetId), startOpenTimeInclusive: startIso, endOpenTimeExclusive: endIso, strategyCode, strategyVersion: strategy.version, strategyParameters: Object.fromEntries(Object.entries(params).map(([key, value]) => [key, Number(value)])), orderAmount, feeRate, forceCloseAtEnd: true });
      onCreated(run); onClose();
    } catch (exception) { setError(exception.message || "创建回测失败"); } finally { setSaving(false); }
  };
  return <aside className="backtest-create-panel" aria-label="新建回测"><div className="backtest-panel-head"><div><span className="eyebrow">NEW EXPERIMENT</span><h3>新建回测</h3></div><button type="button" aria-label="关闭新建回测" onClick={onClose}><X /></button></div><form onSubmit={submit}>
    <label>连续历史数据集<select value={datasetId} onChange={(event) => selectDataset(event.target.value)}><option value="">请选择已校验数据集</option>{datasets.map((item) => <option key={item.datasetId} value={item.datasetId}>{item.symbol} · {item.interval} · {item.candleCount?.toLocaleString()} 根</option>)}</select></label>
...
function RunDetail({ run, loading, error, retry }) {
  ...
  const points = equity?.points || equity?.items || [];
  return <div className="backtest-detail"><section className="backtest-card"><header className="quant-section-head"><h4>任务详情</h4><span className={`backtest-status status-${run.status?.toLowerCase()}`}>{formatRunStatus(run.status)}</span></header><dl className="backtest-detail-grid">{[["runId", run.runId], ["datasetId", run.datasetId], ["交易对 / 周期", `${run.symbol || "—"} · ${run.interval || "—"}`], ["策略", `${run.strategyCode || "—"} · ${run.strategyVersion || "—"}`], ["参数", JSON.stringify(run.strategyParameters || {})], ["回测区间", `${formatInstant(run.startOpenTimeInclusive || run.startOpenTime)} ～ ${formatInstant(run.endOpenTimeExclusive || run.endOpenTime)}`], ["数量 / 手续费", `${run.orderAmount ?? "—"} / ${run.feeRate ?? "—"}`], ["执行模型", run.executionModel], ["K 线数", run.barCount], ["阶段", run.stage || run.status], ["开始 / 完成", `${formatInstant(run.startedAt)} / ${formatInstant(run.completedAt)}`]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value || "—"}</dd></div>)}</dl>{run.status === "FAILED" && <div className="backtest-failure"><strong>{run.errorCode || "FAILED"}</strong><span>{run.errorMessage || "回测失败"}</span></div>}</section>{run.status === "COMPLETED" && <><section className="backtest-metrics">{[["总收益率", formatRatio(metrics.totalReturnRatio), metrics.totalReturnRatio >= 0 ? "positive" : "negative"], ["最大回撤", formatRatio(metrics.maximumDrawdownRatio)], ["胜率", formatRatio(metrics.winRate)], ["交易数", formatDecimal(metrics.tradeCount, 0)], ["净利润", formatDecimal(metrics.netProfit)], ["Profit Factor", formatDecimal(metrics.profitFactor)], ["买入持有", formatRatio(metrics.buyAndHoldReturnRatio)], ["总手续费", formatDecimal(metrics.totalFees)]].map(([label, value, tone]) => <Metric key={label} label={label} value={value} tone={tone} />)}</section><section className="backtest-card"><header className="quant-section-head"><h4><ChartLineUp />权益曲线</h4><small>{equity?.sampled ? `图表展示 ${points.length} / ${equity.totalPoints} 个抽样点，完整结果保存在服务器` : ""}</small></header>{equityError ? <ErrorState label="权益曲线加载失败" error={equityError} retry={() => setEquityError("")} /> : points.length ? <div className="backtest-chart" aria-label="权益和回撤曲线"><ResponsiveContainer width="100%" height="100%"><AreaChart data={points.filter((point) => Number.isFinite(Number(point.equityRatio)) && Number.isFinite(Number(point.drawdownRatio)))}><CartesianGrid stroke="var(--border-subtle)" /><XAxis dataKey="openTime" tickFormatter={(value) => new Date(value).toLocaleDateString("zh-CN")} /><YAxis tickFormatter={(value) => `${(Number(value) * 100).toFixed(0)}%`} /><Tooltip formatter={(value, name) => [formatRatio(value), name === "equityRatio" ? "权益变化" : "回撤"]} labelFormatter={formatInstant} /><Area type="monotone" dataKey="equityRatio" stroke="var(--accent-primary)" fill="var(--accent-primary)" fillOpacity={0.12} /><Area type="monotone" dataKey="drawdownRatio" stroke="var(--accent-red)" fill="var(--accent-red)" fillOpacity={0.08} /></AreaChart></ResponsiveContainer></div> : <div className="backtest-empty">权益曲线不可用</div>}</section></>}</div>;
}

function Trades({ run }) { const [page, setPage] = useState(1); const [state, setState] = useState("idle"); const [data, setData] = useState({ items: [], total: 0 }); useEffect(() => { setPage(1); }, [run?.runId]); useEffect(() => { if (!run?.runId || run.status !== "COMPLETED") return undefined; const controller = new AbortController(); setState("loading"); fetchTrades(run.runId, page, 100, controller.signal).then((result) => { const items = pageItems(result); setData({ items, total: pageTotal(result, items) }); setState("ready"); }).catch((exception) => { if (exception.name !== "AbortError") setState("error"); }); return () => controller.abort(); }, [run, page]); if (run?.status !== "COMPLETED") return null; const totalPages = Math.max(1, Math.ceil(data.total / 100)); return <section className="backtest-card"><header className="quant-section-head"><h4>交易记录</h4><small>{data.total} 条</small></header>{state === "loading" ? <LoadState label="正在读取交易…" /> : state === "error" ? <div className="backtest-empty">交易记录加载失败</div> : <><div className="backtest-table-wrap"><table><thead><tr>{["编号", "入场时间", "入场价", "退出时间", "退出价", "数量", "净利润", "收益率", "持有 Bar", "退出原因"].map((label) => <th key={label}>{label}</th>)}</tr></thead><tbody>{data.items.map((trade) => <tr key={trade.tradeNo}><td>{trade.tradeNo}</td><td>{formatInstant(trade.entryTime)}</td><td>{formatDecimal(trade.entryPrice)}</td><td>{formatInstant(trade.exitTime)}</td><td>{formatDecimal(trade.exitPrice)}</td><td>{formatDecimal(trade.amount)}</td><td>{formatDecimal(trade.netProfit)}</td><td>{formatRatio(trade.returnRatio)}</td><td>{trade.barsHeld ?? "—"}</td><td>{trade.exitReason === "END_OF_SERIES" || trade.forcedExit ? "期末强平" : "策略退出"}</td></tr>)}{!data.items.length && <tr><td colSpan="10" className="backtest-empty">已完成但没有交易</td></tr>}</tbody></table></div><div className="backtest-pagination"><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}><CaretLeft />上一页</button><span>第 {page} / {totalPages} 页</span><button type="button" disabled={page >= totalPages} onClick={() => setPage((value) => value + 1)}>下一页<CaretRight /></button></div></>}</section>; }

export default function QuantBacktests() {
  const [strategies, setStrategies] = useState([]); const [datasets, setDatasets] = useState([]); const [runs, setRuns] = useState([]); const [selectedId, setSelectedId] = useState(null); const [detail, setDetail] = useState(null); const [errors, setErrors] = useState({}); const [loading, setLoading] = useState(true); const [refreshing, setRefreshing] = useState(false); const [showCreate, setShowCreate] = useState(false); const detailRequest = useRef(0);
  const loadLists = useCallback(async (signal) => { setRefreshing(true); setErrors({}); const [strategyResult, datasetResult, runResult] = await Promise.allSettled([fetchStrategies(signal), fetchDatasets(signal), fetchRuns(1, 20, signal)]); if (strategyResult.status === "fulfilled") setStrategies(pageItems(strategyResult.value)); else if (strategyResult.reason?.name !== "AbortError") setErrors((current) => ({ ...current, strategies: strategyResult.reason.message })); if (datasetResult.status === "fulfilled") setDatasets(pageItems(datasetResult.value)); else if (datasetResult.reason?.name !== "AbortError") setErrors((current) => ({ ...current, datasets: datasetResult.reason.message })); if (runResult.status === "fulfilled") setRuns(pageItems(runResult.value)); else if (runResult.reason?.name !== "AbortError") setErrors((current) => ({ ...current, runs: runResult.reason.message })); setLoading(false); setRefreshing(false); }, []);
  ...
  useEffect(() => { if (selectedId) return loadDetail(selectedId); return undefined; }, [selectedId, loadDetail]);
  ...
  useEffect(() => { let timer; let active = false; const poll = async () => { if (document.visibilityState !== "visible" || active) return; active = true; try { const result = await fetchNonTerminalRuns(); if (pageItems(result).length) await refresh(); } finally { active = false; } }; const schedule = () => { clearInterval(timer); if (document.visibilityState === "visible") { poll(); timer = window.setInterval(poll, 3000); } }; document.addEventListener("visibilitychange", schedule); schedule(); return () => { document.removeEventListener("visibilitychange", schedule); clearInterval(timer); }; }, [refresh]);
  ...
  return <QuantPageScaffold pageClass="quant-backtests-page"><div className="quant-workspace-head"><div><span className="eyebrow">QUANT · BACKTEST LAB</span><h3>回测实验</h3><small>基于已校验历史数据运行确定性策略回测</small></div><div className="backtest-head-actions"><button type="button" className="quant-refresh" onClick={refresh} disabled={refreshing}><ArrowsClockwise className={refreshing ? "spin" : ""} />刷新</button><button type="button" className="quant-primary-action" onClick={() => setShowCreate(true)}><Flask />新建回测</button></div></div>
    ...
    {errors.strategies && <div className="backtest-notice" role="alert">策略不可用：{errors.strategies}</div>}{errors.datasets && <div className="backtest-notice" role="alert">数据集不可用：{errors.datasets}</div>}{errors.runs && <ErrorState label="任务列表加载失败" error={errors.runs} retry={refresh} />}
    {loading ? <LoadState label="正在读取回测工作台…" /> : <div className="backtest-main-grid"><section className="backtest-card backtest-run-list"><header className="quant-section-head"><h4>回测任务</h4><small>{runs.length ? "当前页 20 条以内" : "还没有创建回测"}</small></header>{!runs.length ? <div className="backtest-empty">还没有创建回测</div> : runs.map((run) => <button type="button" className={`backtest-run-row ${String(run.runId) === String(selectedId) ? "selected" : ""}`} key={run.runId} onClick={() => setSelectedId(run.runId)}><span className={`backtest-status status-${run.status?.toLowerCase()}`}>{formatRunStatus(run.status)}</span><strong>{run.symbol || "—"} · {run.interval || "—"}</strong><span>{run.strategyCode || "—"}</span><span>{formatInstant(run.startOpenTimeInclusive || run.startOpenTime)} ～ {formatInstant(run.endOpenTimeExclusive || run.endOpenTime)}</span><span>{run.status === "COMPLETED" ? formatRatio(run.metrics?.totalReturnRatio) : "—"} · 回撤 {run.status === "COMPLETED" ? formatRatio(run.metrics?.maximumDrawdownRatio) : "—"}</span></button>)}</section><div><RunDetail run={detail || selectedRun} loading={Boolean(selectedId) && !detail && !errors.detail} error={errors.detail} retry={() => loadDetail(selectedId)} /><Trades run={detail || selectedRun} /></div></div>}
    {showCreate && <CreatePanel strategies={strategies} datasets={datasets} onClose={() => setShowCreate(false)} onCreated={(run) => { setSelectedId(run.runId); refresh(); }} />}
  </QuantPageScaffold>;
}
