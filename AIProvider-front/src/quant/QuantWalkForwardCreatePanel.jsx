...
import {
  ...
  parseIntegerCsv,
  splitDataset7030,
  calculateWalkForwardWindow,
  WALK_FORWARD_SELECTION_METRIC_LABELS,
...
export default function QuantWalkForwardCreatePanel({
  strategies,
  datasets,
  executionProfiles = [],
  onClose,
  ...
  useEffect(() => {
    if (!dataset) return;
    setStudyStart(utcInstantToLocalInput(dataset.earliestOpenTime));
    const duration = intervalDurationMs(
      dataset.intervalCode || dataset.interval,
    );
    if (duration > 0)
      setStudyEnd(
        utcInstantToLocalInput(
  ...
  const submit = async (event) => {
    ...
    if (!contextResult.valid || !parsedGrid.grid || parsedGrid.error)
      return setError(
        parsedGrid.error ||
          "请完整选择兼容的市场、数据集、策略和执行模型",
      );
    if (
      candidateCount == null ||
      candidateCount < 1 ||
      ...
      return setError("候选组合必须为 1～64 个");
    const windowResult = calculateWalkForwardWindow({
      studyStart: toUtcIso(studyStart),
      studyEnd: toUtcIso(studyEnd),
      dataset,
      trainingBars: Number(trainingBars),
    ...
    if (windowResult.error) return setError(windowResult.error);
    ...
    const normalizedFee = normalizeDecimalString(feeRate, {
      maxIntegerDigits: 1,
      maxFractionDigits: 18,
    });
    ...
    if (
      !normalizedFee ||
      compareDecimalStrings(normalizedFee, "0") < 0 ||
      compareDecimalStrings(normalizedFee, "0.01") > 0
    )
      return setError("feeRate 必须为 0～0.01，最多 18 位小数");
    const body = {
      ...
      studyStartOpenTimeInclusive: toUtcIso(studyStart),
      studyEndOpenTimeExclusive: toUtcIso(studyEnd),
      trainingBars: Number(trainingBars),
      validationBars: Number(validationBars),
      selectionMetric,
    ...
    try {
      ...
      if (
        ...
        setError(exception.message || "创建滚动验证失败");
    } finally {
  ...
  return (
    <aside
      className="backtest-create-panel quant-walk-forward-create"
      role="dialog"
      aria-modal="true"
      aria-label="新建滚动验证"
    >
      <div className="backtest-panel-head">
        <div>
          <span className="eyebrow">WALK-FORWARD STUDY</span>
          <h3>新建滚动验证</h3>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={submit}>
        ...
        <div className="backtest-form-grid">
          ...
          <label>
            ...
            <input
              ...
              type="datetime-local"
              value={studyEnd}
              onChange={(event) => setStudyEnd(event.target.value)}
            />
          </label>
        </div>
        <div className="backtest-form-grid">
          <label>
            trainingBars
            <input
              aria-label="trainingBars"
              inputMode="numeric"
              value={trainingBars}
              onChange={(event) => setTrainingBars(event.target.value)}
            />
          </label>
          <label>
            validationBars
            <input
              aria-label="validationBars"
              inputMode="numeric"
              value={validationBars}
              onChange={(event) => setValidationBars(event.target.value)}
            />
          </label>
        </div>
        <button
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => setSelectionMetric(event.target.value)}
          >
            {Object.entries(WALK_FORWARD_SELECTION_METRIC_LABELS).map(
              ([key, label]) => (
        ...
        <label>
          minimumTrainTrades
          <input
            aria-label="minimumTrainTrades"
            inputMode="numeric"
            value={minimumTrainTrades}
            onChange={(event) => setMinimumTrainTrades(event.target.value)}
          />
        </label>
        <div className="backtest-form-grid">
          <label>
        ...
        <div className="quant-walk-forward-window-summary">
          <strong>滚动模式：ROLLING</strong>
          <span>
            步长：{validationBars || "—"} · Fold 数量：
            {derived.error ? "—" : derived.foldCount || "—"}
          </span>
          <span>
            每 Fold 候选：{candidateCount || "—"} · 总回测任务：
            {derived.error ? "—" : derived.totalChildRuns || "—"}
          </span>
          {derived.error ? (
            <small>{derived.error}</small>
          ) : (
            <small>
              首个 VALIDATION 结束：{derived.firstValidationEnd || "—"} ·
              最后结束：{derived.lastValidationEnd || "—"}
            </small>
          )}
        </div>
        {strategy && (
          <p className="backtest-help">
            ...
            线；网格中更大周期参数的最终要求由后端逐组合校验。
          </p>
        )}
        <p className="backtest-help">
          候选只按 TRAIN 指标选择；VALIDATION
          结果不参与选择。样本外结果不代表未来收益。
        </p>
        {error && (
          <p className="backtest-inline-error" role="alert">
            {error}
