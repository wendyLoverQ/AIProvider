...
import { formatDirectionMode, formatOrderSizingMode } from "./quantExecutionContext";

export default function QuantWalkForwardStudyDetail({ detail, executionProfile, loading, error, onRetry }) {
  ...
  return <section className="backtest-card quant-walk-forward-detail"><header className="quant-section-head"><div><span className="eyebrow">WALK-FORWARD STUDY</span><h4>Study 详情</h4></div><span className={`backtest-status status-${study.status.toLowerCase()}`}>{WALK_FORWARD_STUDY_STATUS_LABELS[study.status]}</span></header>
    <dl className="backtest-detail-grid"><div><dt>studyId</dt><dd className="copyable-id">{study.studyId}</dd></div><div><dt>数据集</dt><dd>{study.symbol} · {intervalCode(study.intervalCode)} · {study.provider}</dd></div><div><dt>策略</dt><dd>{study.strategyCode} · {study.strategyVersion}</dd></div><div><dt>执行模型</dt><dd>{executionProfile?.name || study.executionProfileCode}</dd></div><div><dt>方向</dt><dd>{formatDirectionMode(study.directionMode)}</dd></div><div><dt>规模模式</dt><dd>{formatOrderSizingMode(study.orderSizingMode)}</dd></div><div><dt>窗口</dt><dd>ROLLING · TRAIN {study.trainingBars} · VALIDATION {study.validationBars} · STEP {study.stepBars}</dd></div><div><dt>研究区间</dt><dd>{formatInstant(study.studyStartOpenTimeInclusive)} ～ {formatInstant(study.studyEndOpenTimeExclusive)}</dd></div><div><dt>Fold / child runs</dt><dd>{study.foldCount} / {study.totalChildRuns}</dd></div><div><dt>候选 / 选择指标</dt><dd>{study.candidateCountPerFold} / {WALK_FORWARD_SELECTION_METRIC_LABELS[study.selectionMetric]}</dd></div><div><dt>最低训练交易</dt><dd>{study.minimumTrainTrades}</dd></div><div><dt>进度</dt><dd>{study.progressPercent}% · {study.completedFolds} 完成 / {study.failedFolds} 失败</dd></div><div><dt>基础资产数量 / 手续费</dt><dd>{study.orderAmount} / {study.feeRate}</dd></div><div><dt>创建 / 开始 / 完成</dt><dd>{formatInstant(study.createdAt)} / {formatInstant(study.startedAt)} / {formatInstant(study.finishedAt)}</dd></div></dl>
