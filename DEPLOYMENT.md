...
首次使用时在页面点击“开始设备登录”，按显示的 OpenAI 设备授权地址与代码完成一次登录。后端直接运行 Codex CLI；未登录、命令不可用或执行失败时明确返回错误，不切换到其他实现。远程对话按管理员要求使用 `--dangerously-bypass-approvals-and-sandbox`，新对话与续聊都拥有服务器 `ubuntu` 用户的完整文件、终端和网络权限，因此必须保留独立访问密钥并限制公网入口。
